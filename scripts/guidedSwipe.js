...
import { extension_settings, getContext } from '../../../../extensions.js'; // Import getContext and extension_settings
import { getPreviousImpersonateInput, setPreviousImpersonateInput } from '../index.js'; // Import shared state functions
    ...
    if (typeof SillyTavern !== 'undefined' && typeof SillyTavern.getContext === 'function') {
      ...
      if (typeof context.executeSlashCommandsWithOptions === 'function') {
        ...
        await context.executeSlashCommandsWithOptions(command, {
          /* options if needed, e.g., showOutput: false */
        });
      } else {
  ...
  try {
    ...
    if (messageData && Array.isArray(messageData.swipes) && messageData.swipes.length > 1) {
      ...
      if (messageData.swipe_id !== targetSwipeIndex) {
        console.log(
          `[GuidedGenerations][Swipe] Navigating directly from swipe ${messageData.swipe_id} to last swipe ${targetSwipeIndex}.`,
        );
        ...
        if (mesDom) {
          ...
          [...mesDom.querySelectorAll('.swipes-counter')].forEach(
            it => (it.textContent = `${messageData.swipe_id + 1}/${messageData.swipes.length}`),
          );
        } else {
          console.warn(
            `[GuidedGenerations][Swipe] Could not find DOM element for message ${lastMessageIndex} to update UI during direct navigation.`,
          );
        }
      ...
      console.log(
        '[GuidedGenerations][Swipe] No existing swipes or only one swipe found. Proceeding to generate first/next swipe.',
      );
    }
    ...
    const generationPromise = new Promise((resolve, reject) => {
      ...
      timeoutId = setTimeout(() => {
        if (!resolved) {
          ...
          console.error(
            `[GuidedGenerations][Swipe] Swipe generation timed out after ${timeoutDuration / 1000} seconds.`,
          );
...
const guidedSwipe = async () => {
  ...
  // Clear the textarea immediately after saving input
  textarea.value = '';
  console.log('[GuidedGenerations][Swipe] Textarea cleared temporarily.');

  try {
    // Use user-defined guided swipe prompt override
    const promptTemplate = extension_settings[extensionName]?.promptGuidedSwipe ?? '';
    // Use originalInput here, as textarea is now cleared
    const filledPrompt = promptTemplate.replace('{{input}}', originalInput);

    // --- 1. Inject Context (if any) --- (Use direct context method)
    if (originalInput.trim()) {
      // Use the currentInjectionRole retrieved above
      const stscriptCommand = `// Guided Swipe logic|
    ...
    if (!injectionFound) {
      const errorMsg =
        "[GuidedGenerations][Swipe] Critical Error: Guided instruction injection ('script_inject_instruct') failed to appear in context after multiple checks.";
      console.error(errorMsg);
      alert(
        "Guided Swipe Error: Could not verify instruction injection ('script_inject_instruct'). Aborting swipe generation.",
      );
    ...
    if (textarea) {
      // Check if textarea was found initially
